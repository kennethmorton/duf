classdef dufEventSetHmds < dufEventSet
    
    properties
        aggregateOnLink = true;
    end
    ...
    methods
        function self = dufEventSetHmds(varargin)
    ...
    methods
        function self = link(self)
            ...
            [uMdrFiles, lastOccurance, collectionIndex] = unique({allCollectionInfo.mdrFile}');
            
            nExcursions = length(uMdrFiles);
            
            eventsOut = cell(nExcursions,1);
            %self.excursionArray = cell(nExcursions,1);
            allX = cell(nExcursions,1);
            for iEx = 1:nExcursions
                cCollectionInfo = allCollectionInfo(lastOccurance(iEx));
                
                cEvents = self.observationInfo(collectionIndex==iEx);
                cX = self.X(collectionIndex==iEx,:);
                
                cAscFile = cCollectionInfo.ascFile;
                cMdrFile = cCollectionInfo.mdrFile;
                
                alarmSet = fromGpsLocAndConf(gprAlarmSet,cat(2, cat(1,cEvents.xUTM), cat(1,cEvents.yUTM)), cat(1, cEvents.confidence));
                pos = readMdrScanHeadersCache(cMdrFile);
                alarmSet.extent = gprScoreNiitekPosToExtent(pos);
                objectSet = ascRead(dufObjectSet,cAscFile);
                
                cExcursion = gprExcursion('alarmSet',alarmSet,'objectSet',objectSet);
                cExcursion = cExcursion.link;
                cExcursion = cExcursion.intersect;
                
                self.excursionSummaries(iEx).collectionInfo = cCollectionInfo;
                self.excursionSummaries(iEx).scoreableArea = cExcursion.intersectedExtent.getArea;
                self.excursionSummaries(iEx).extent = cExcursion.intersectedExtent;
                
                for iEvent = 1:length(cEvents)
                    cObjects = cExcursion.objectSet.objectArray(logical(cExcursion.linkedAlarmsObjects(iEvent,:)));
                    
                    cEvents(iEvent).isOnObject = ~isempty(cObjects);
                    cEvents(iEvent).isMiss = false;
                    cEvents(iEvent).objects = cObjects;
                    cEvents(iEvent).excursionId = iEx;
                end
                
                isMissed = ~full(sum(cExcursion.linkedAlarmsObjects));
                
                nMisses = sum(isMissed);
                misObjects = cExcursion.objectSet.objectArray(isMissed);
                misEvents = repmat(cEvents(1),nMisses,1);
                if nMisses > 0
                    posMat = as.niitekPos2PosMat(pos,51);
                    for iEvent = 1:nMisses
                    
                        cObject = misObjects(iEvent);
                        cCenter = cObject.extent.getCenter;
                        dtxt = gpsToDtXt(cCenter,posMat);% nChannles
                    
                        misEvents(iEvent).confidence = nan;
                        misEvents(iEvent).xUTM = cCenter(1);
                        misEvents(iEvent).yUTM = cCenter(2);
                        misEvents(iEvent).dt = dtxt(1);
                        misEvents(iEvent).xt = dtxt(2);
                        misEvents(iEvent).isOnObject = true;
                        misEvents(iEvent).isMiss = true;
                        misEvents(iEvent).objects = cObject;
                        misEvents(iEvent).excursionId = iEx;
                    end
                end
                
                allX{iEx} = cat(1,cX,nan(nMisses,size(cX,2)));
                
                eventsOut{iEx} = cat(1,cEvents, misEvents);
                
                self.excursions{iEx} = cExcursion;
            end
            
            self.observationInfo = [];
            self.X = cat(1,allX{:});
            self.observationInfo = cat(1,eventsOut{:});
...
%             end
%             
%             title(subplotContainerAxes([0 0 0 0.02]),titleString,'FontSize',20,'Interpreter','none'); % expand vertically by 0.02 normalized units
%                             
%             % Position string
